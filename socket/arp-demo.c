...
const char*format_mac(const void*data,char buf[18]){
    static __thread char innerbuf[18];
    char*p=buf?:innerbuf;
    const unsigned char*q=data;
    sprintf(p,"%02X:%02X:%02X:%02X:%02X:%02X",q[0],q[1],q[2],q[3],q[4],q[5]);
    return p;
}
...
int read_arp(int fd)
{
    ...
    union {struct in_addr inaddr;unsigned char p[sizeof(struct in_addr)];} ip={};
    memcpy(ip.p,arp_resp->sender_ip,sizeof(ip));
    debug("@sender %s,%s",inet_ntoa(ip.inaddr),format_mac(arp_resp->sender_mac,0));
    memcpy(ip.p,arp_resp->target_ip,sizeof(ip));
    debug("@target %s,%s",inet_ntoa(ip.inaddr),format_mac(arp_resp->target_mac,0));
    ret = 0;
